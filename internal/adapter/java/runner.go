...
func (r execRunner) Run(parent context.Context, executable string, args []string, directory string, environment map[string]string) (string, error) {
	timeout := r.timeout
	if timeout <= 0 {
	...
	command.Env = controlledEnvironment(environment)
