...
**I00：产品契约冻结** 至 **I14：通用受控执行器与操作日志** 已按顺序通过验收。系统可以将本机、项目和新鲜官方版本事实组合成 Node/Java 的结构化建议，把一项合格的 NVM Node 更新建议转换为只读计划预览，并通过内部确定性核心执行一个经过确认的无害自检动作。公开 CLI 仍不能安装、升级、卸载或修改系统。
...
I14 新增内部受控执行核心、[`Plan 0.2.0`](./schemas/plan/v0.2.0.json) 和 [`Operation Record 0.1.0`](./schemas/operation/v0.1.0.json)。Plan 仍只携带声明式动作身份，不包含命令或参数；实际进程规范只能来自核心内置注册表，使用绝对路径和结构化参数直接执行，不经过 Shell。执行前必须重新校验不可变 Plan ID、30 分钟有效期和绑定相同 Plan ID 的用户确认。

当前唯一可执行动作是内部 R1 自检：调用项目自身固定的 `version` 命令并验证结果，再写入脱敏且有长度上限的本地 JSON 审计记录。它不安装软件、不调用包管理器、不提权，也没有公开 `apply` 入口。现有 `envmason plan` 仍生成 `0.1.0` 的不可执行 Node/NVM 预览，不能被 I14 执行器复用。

操作记录遵循平台原生状态目录：macOS 为 `~/Library/Application Support/EnvMason/operations`，Windows 为 `%LOCALAPPDATA%/EnvMason/operations`，Linux 为 `$XDG_STATE_HOME/envmason/operations`，未设置时回退到 `~/.local/state/envmason/operations`。stdout、stderr 分别限制为 64 KiB；完成状态要求进程成功且注册验证器通过，中断遗留状态不会被报告为 Completed。
