...
**I00：产品契约冻结** 至 **I15：单个 NVM Node 版本安装** 已按顺序通过验收。系统可以将本机、项目和新鲜官方版本事实组合成 Node/Java 的结构化建议，把一项合格的 Node 目标转换为可审查 Plan，并在 macOS 上通过现有 NVM 安装一个经用户逐字确认的精确 Node 版本。当前仍不能安装 NVM、切换默认版本、升级 npm/pnpm、卸载旧版本或执行任意命令。
