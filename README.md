...
**I00：产品契约冻结** 至 **I09：通用版本规范化与比较** 已按顺序通过验收，首个 macOS 只读预览里程碑具备可运行能力，内部核心可以保守比较 SemVer/Node 与常见 Java 版本。远程版本评估、配置和系统修改能力尚未开始。
...
I08 提供首个 macOS 综合只读报告。默认终端摘要以及 Markdown、JSON 输出都来自同一次确定性清单模型；JSON 输出继续遵循 Inventory Schema `0.2.0`。
...
重复的类别或严重程度值采用 OR，类别与严重程度之间采用 AND。部分适配器失败时仍会生成报告，并用 `REPORT_SECTION_FAILED` 和 `REPORT_INCOMPLETE` 标记不完整；命令不会联网查询版本，也不会修改包管理器、配置或系统。

I09 的版本规范化与比较目前是内部确定性核心，尚未增加独立 CLI 或将比较结论接入报告；这部分用户可见能力按后续增量开放。
