...
**I00：产品契约冻结** 至 **I07：Java 生态只读适配器** 已按顺序通过验收。当前已具备确定性的内部系统、PATH、Homebrew、Node.js 与 Java 生态只读发现能力；I08 统一只读报告正在验收，配置和系统修改能力尚未开始。
...
I08 提供首个 macOS 综合只读报告候选。默认终端摘要以及 Markdown、JSON 输出都来自同一次确定性清单模型；JSON 输出继续遵循 Inventory Schema `0.2.0`。
